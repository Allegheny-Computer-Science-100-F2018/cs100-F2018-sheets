...
like
``Allegheny-Computer-Science-100-Fall-2018/computer-science-100-fall-2018-lab-3-gkapfham''.
...
\begin{lstlisting}
  git clone https://github.com/Allegheny-Computer-Science-100-F2018/computer-science-100-fall-2018-lab-3-gkapfham.git
\end{lstlisting}
...
% normally use the text editor called \command{gvim}.

Today, you will view and improve another Java program in {\tt gvim}! To load the
source code of today's Java program into \program{gvim}, you should type
\command{gvim src/main/java/labthree/TipCalculator.java} in your terminal
window. This should cause a new window, the \program{gvim} text editor, to
appear on your screen. Since \program{gvim} is a modal editor, you may notice
that if you start typing, nothing appears (unless you happen to hit certain
letters such as \command{i}, \command{o}, \command{a}, and a few others). This
is because you are not in ``insert mode.'' To get into insert mode, just type
the letter \command{i} (lower case). Once you do this, the window should show
\command{--INSERT--} in the lower left corner.

This assignment invites you to implement a Java program that displays exactly
ten lines of output. The first output line should display your name and then
today's date. You can accomplish the first task by typing
...
produce nine additional lines of output once you add eight more
...
\noindent If a steps says to ``ask the user'' this means the program should read
from a file. A sample run is:

\begin{verbatim}
  Gregory M. Kapfhammer Thu Sep 14 12:50:07 EDT 2018
...
today's date, and nine more lines of text. Make sure there are spaces separating
...
As verified by the Checkstyle tool, the source code for the \mainprogram{} file
...
submission meets the standards for correctness.
...
checks. For instance, GatorGrader will check to ensure that \mainprogram{}
...
then checking to see that the transfer to GitHub is successful. If you notice
that the network communication with GitHub did not work correctly, then please
try to determine why, asking a teaching assistant or the instructor for
