...
% add a call to the {\tt page.fillRect} method so that it
...
% of the one requested by the user.

% Now, the source code line ``{\tt page.setColor(userComplementaryColor);}'' will
% cause the next shape to be filled with the complement of the color chosen by the
% user. Finally, you will need to again call the {\tt page.fillRect} method with
% the parameters that will yield a rectangle correctly drawn in the second half of
% the image. Please see the instructor or a teaching assistant if you have
...
% successful. Of course, you will also need to use \command{git} commands in your
% terminal to transfer the \secondprogramsource{} to GitHub. If you notice that
